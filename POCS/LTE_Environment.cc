...
std::vector < double >
    convertStringtoDouble(std::string cioList, uint16_t rep) {
        std::vector < double > v;
        double tempVal;
...
int main(int argc, char * argv[]) {
    ...
    Config::SetDefault("ns3::UdpClient::PacketSize", UintegerValue(1200)); // bytes
    Config::SetDefault("ns3::UdpClient::MaxPackets", UintegerValue(10000000));
    Config::SetDefault("ns3::LteRlcUm::MaxTxBufferSize", UintegerValue(10 * 1024));
    Config::SetDefault("ns3::ConfigStore::Filename", StringValue("LTE_Attributes.txt"));
    ...
    NodeContainer EdgeUes;
    EdgeUes.Create(nEdgeUes);
    NodeContainer EdgeUes2;
    EdgeUes2.Create(nEdgeUes2);
    ...
    NS_LOG_LOGIC(" ====== Setting handover parameters ====== ");
    NS_LOG_LOGIC("Hysteresis = " << hysteresisCoefficient << "dB");
    NS_LOG_LOGIC("TimeToTrigger = " << timeToTrigger << "ms");
    ...
    if (!fadingTrace.empty()) {
        ...
        NS_LOG_LOGIC("Filename " << fadingTrace);
    } else {
        NS_LOG_LOGIC("No fading specified");
    }
    ...
    NS_LOG_LOGIC(" ======macroEnbTxPowerDbm ====== " << macroEnbTxPowerDbm);
    ...
    Ptr < MyGymEnv > myGymEnv = CreateObject < MyGymEnv > (envStepTime, nMacroEnbSites, nEdgeUes + nEdgeUes2 + nCenterUes, macroEnbBandwidth,macroEnbDevs);
    ...
    for (uint32_t it = 0; it != macroEnbs.GetN(); ++it) {
        ...
        NS_LOG_LOGIC("Cell #" << enbNetDevice -> GetCellId() << " Pos =  " << tempLocation << ", CIO = " << cioListDouble[it]);
    }

    // this enables handover for macro eNBs
    lteHelper -> AddX2Interface(macroEnbs);
    NS_LOG_LOGIC(" ====== Deploying Edge UEs ====== ");
    MobilityHelper EdgeUesMobility;
    int64_t m_streamIndex1 = 0;
    ObjectFactory pos1;
    pos1.SetTypeId("ns3::RandomBoxPositionAllocator");
    pos1.Set("X", StringValue("ns3::UniformRandomVariable[Min=250.0|Max=252.0]"));
    ...
    EdgeUesMobility.SetMobilityModel("ns3::RandomWalk2dMobilityModel",
        ...
        "Bounds", StringValue("250|252|-25|25"));
    ...
    for (uint32_t it = 0; it != EdgeUes.GetN(); ++it) {
        ...
        Ptr < NetDevice > netDevice = EdgeUeDevs.Get(it);
        Ptr < LteUeNetDevice > uebNetDevice = netDevice -> GetObject < LteUeNetDevice > ();
        Ptr < MobilityModel > mobilityModel = node -> GetObject < MobilityModel > ();
        tempLocation = mobilityModel -> GetPosition();
        eNBsLocation.push_back(tempLocation);
        NS_LOG_LOGIC("HUe #" << uebNetDevice -> GetImsi() << " Pos =  " << tempLocation);
    }

    NS_LOG_LOGIC(" ====== Deploying Edge UEs2 ====== ");
    MobilityHelper EdgeUesMobility2;

    int64_t m_streamIndex2 = 0;
    ObjectFactory pos2;
    pos2.SetTypeId("ns3::RandomBoxPositionAllocator");
    pos2.Set("X", StringValue("ns3::UniformRandomVariable[Min=748.0|Max=750.0]"));
    ...
    EdgeUesMobility2.SetMobilityModel("ns3::RandomWalk2dMobilityModel",
        ...
        "Time", StringValue("0.2s"),
        "Speed", StringValue("ns3::ConstantRandomVariable[Constant=3.0]"),
        "Bounds", StringValue("748|750|-25|25"));
    ...
    NetDeviceContainer EdgeUeDevs2 = lteHelper -> InstallUeDevice(EdgeUes2);

    for (uint32_t it = 0; it != EdgeUes2.GetN(); ++it) {
        Ptr < Node > node = EdgeUes2.Get(it);
        Ptr < NetDevice > netDevice = EdgeUeDevs2.Get(it);
        Ptr < LteUeNetDevice > uebNetDevice = netDevice -> GetObject < LteUeNetDevice > ();
        Ptr < MobilityModel > mobilityModel = node -> GetObject < MobilityModel > ();
        tempLocation = mobilityModel -> GetPosition();
        eNBsLocation.push_back(tempLocation);
        NS_LOG_LOGIC("2Ue #" << uebNetDevice -> GetImsi() << " Pos =  " << tempLocation);
    }
    // Center Ues
    NS_LOG_LOGIC(" ====== Deploying Center UEs ====== ");
    NodeContainer CenterUes;
    CenterUes.Create(nCenterUes);
    ...
    NetDeviceContainer CenterUeDevs = lteHelper -> InstallUeDevice(CenterUes);

    for (uint32_t it = 0; it != CenterUes.GetN(); ++it) {
        Ptr < Node > node = CenterUes.Get(it);
        Ptr < NetDevice > netDevice = CenterUeDevs.Get(it);
        Ptr < LteUeNetDevice > uebNetDevice = netDevice -> GetObject < LteUeNetDevice > ();
        Ptr < MobilityModel > mobilityModel = node -> GetObject < MobilityModel > ();
        tempLocation = mobilityModel -> GetPosition();
        eNBsLocation.push_back(tempLocation);
        NS_LOG_LOGIC("Ue #" << uebNetDevice -> GetImsi() << " Pos =  " << tempLocation);
    }

    NS_LOG_LOGIC(" ====== Deployment Summary ======");
    for (uint16_t i = 0; i < CenterUes.GetN(); ++i) {
        Ptr < Node > node = CenterUes.Get(i);
        Ptr < MobilityModel > mobilityModel = node -> GetObject < MobilityModel > ();

        NS_LOG_LOGIC("Node# " << node -> GetId() << " is initially at " << mobilityModel -> GetPosition());
    }
    ...
    ues.Add(EdgeUes2);
    ues.Add(CenterUes);
    ueDevs.Add(EdgeUeDevs);
    ueDevs.Add(EdgeUeDevs2);
    ueDevs.Add(CenterUeDevs);
    ...
    lteHelper -> Attach(CenterUeDevs);
    lteHelper -> Attach(EdgeUeDevs);
    lteHelper -> Attach(EdgeUeDevs2);
